...
void BluetoothHandler::exportProfiles() {
    ...
    // --- AQUÍ ESTÁ LA MAGIA: XML SERVICE RECORD ---
    // Esto grita "SOY ANDROID AUTO" en el idioma que el teléfono entiende.
    registerProfile(AAWG_PROFILE_OBJECT_PATH, AAWG_PROFILE_UUID, {
        {"Name", DBus::Variant("AA Wireless")},
        {"Role", DBus::Variant("server")},
        // --- CAMBIO 1: Forzamos el Canal 10 en DBus ---
        {"Channel", DBus::Variant(uint16_t(10))}, 
        {"ServiceRecord", DBus::Variant(std::string(
            ...
            "      <sequence>\n"
            "        <uuid value=\"0x0003\" />\n"
            "        // --- CAMBIO 2: Forzamos el Canal 10 en el XML --- \n"
            "        <uint8 value=\"10\" />\n"
            "      </sequence>\n"
            "    </sequence>\n"
            "  </attribute>\n"
            "  <attribute id=\"0x0100\">\n"
            "    <text value=\"AA Wireless\" />\n"
            "  </attribute>\n"
            "</record>"
        ))}
    });
    ...
    Logger::instance()->info("Bluetooth AA Wireless profile active (XML Injected)\n");
...
void BluetoothHandler::connectDevice() {
    ...
    // We only want to actively connect if we are NOT in Dongle Mode, 
    // OR if we are forcing a retry in Dongle Mode.
    // However, the caller (retryConnectLoop) now controls the policy.
    ...
    for (const std::string &device_path: device_paths) {
        ...
        // We go back to ConnectProfile because it's friendlier to the Android Bluetooth stack
        ...
        try {
            if (deviceConnected && deviceConnected->value()) {
                 // If already connected, we assume it's good. We don't force disconnect here 
                 // because we might kill a connection the phone just started.
                 Logger::instance()->info("Bluetooth device already connected. Skipping connect attempt.\n");
                 continue;
            }

            // We try to connect to the AAWireless UUID specifically.
            // This is the "Service" we registered in XML.
            // If we connect to THIS profile, we trigger AA without needing audio profiles.
            Logger::instance()->info("Connecting to AAWireless Service UUID...\n");
            connectProfile(AAWG_PROFILE_UUID); 
            
            Logger::instance()->info("Bluetooth connection command sent\n");

        } catch (DBus::Error& e) {
             Logger::instance()->info("Failed to connect device at path: %s - Error: %s\n", device_path.c_str(), e.name());
        }
    }
}
...
void BluetoothHandler::retryConnectLoop() {
    ...
    Logger::instance()->info("Starting Connection Loop. Strategy: %d\n", Config::instance()->getConnectionStrategy());
    ...
    while (!should_exit) {
        bool isDongleMode = (Config::instance()->getConnectionStrategy() == ConnectionStrategy::DONGLE_MODE);

        if (isDongleMode) {
            // HYBRID LOGIC:
            // 1. Wait passively for the phone to see Advertisement (preferred)
            // 2. If phone is lazy (doesn't connect in ~15s), poke it.
            
            if (wait_cycles < 3) {
                Logger::instance()->info("Waiting for phone to connect (Passive Advertising)... Cycle %d/3\n", wait_cycles + 1);
                // Wait 5 seconds
                if (connectWithRetryFuture.wait_for(std::chrono::seconds(5)) == std::future_status::ready) {
                    should_exit = true;
                }
                wait_cycles++;
                continue; // Loop again, don't connect yet
            }
            
            // If we reach here, 15 seconds passed and no connection.
            Logger::instance()->info("Phone lazy? Switching to Active Connection attempt.\n");
        }

        // Active connection attempt (either because we are not in dongle mode, or timed out waiting)
        connectDevice();
        ...
        // Reset cycles to wait passively again after a failed active attempt
        if (isDongleMode) { 
            wait_cycles = 0; 
        }
    }
