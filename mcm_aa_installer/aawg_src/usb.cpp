...
void UsbManager::writeGadgetFile(std::string gadgetName, std::string relativeFilePath, const char* content) {
    ...
    if (gadgetFile != NULL) {
        ...
        // Imprimimos el error en la consola pero NO crasheamos
        fprintf(stderr, "[ADVERTENCIA] No se pudo escribir en: %s\n", gadgetFilePath.c_str());
    }
...
void UsbManager::switchToAccessoryGadget() {
    ...
    Logger::instance()->info("USB Manager: Waiting 1000ms for hardware release...\n");
