#!/bin/bash
# install.sh - Instalador Automático V6 (Turbo Boot & AudioKill)
...
apt-get update || true
apt-get install -y hostapd dnsmasq bluetooth bluez bluez-tools iw libusb-1.0-0 libssl-dev libprotobuf-dev protobuf-compiler iptables || echo "ADVERTENCIA: Falló apt-get. Asumiendo dependencias preinstaladas..."
...
# Lista ampliada para arranque rápido (Docker, Modem, Usuario, Impresoras)
SERVICES_TO_KILL="adbd android-tools-adbd usb-gadget serial-getty@ttyGS0 serial-getty@ttyUSB0 dnsmasq avahi-daemon systemd-resolved pulseaudio pipewire wireplumber ModemManager NetworkManager-wait-online cups cups-browsed unattended-upgrades snapd docker containerd user@1000 man-db man-db.timer arduino-router blueman-mechanism udisks2 accounts-daemon polkit debos-grow-rootfs"
...
# 1. Quitar Timeout del Bootloader
...
# 2. Silenciar Kernel y Saltar FSCK
