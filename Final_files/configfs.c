...
// Definimos punteros con nombres UNICOS para evitar choque con f_accessory.c
static int (*acc_ctrlrequest_ptr)(struct usb_composite_dev *cdev, const struct usb_ctrlrequest *ctrl) = NULL;
static void (*acc_disconnect_ptr)(void) = NULL;
...
void register_acc_handlers(
    ...
    // Asignamos los handlers a nuestras variables internas renombradas
    acc_ctrlrequest_ptr = ctrlrequest_handler;
    acc_disconnect_ptr = disconnect_handler;
}
...
#ifdef CONFIG_USB_CONFIGFS_F_ACC
    if (value < 0 && acc_ctrlrequest_ptr != NULL)
        value = acc_ctrlrequest_ptr(cdev, c);
#endif
