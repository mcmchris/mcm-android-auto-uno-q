...
# V9: Disable IPv6 (Fixes "Duplicate Address" crashes) + V8 Fixes
...
# Fix Bluetooth "Zombie"
sudo rfkill unblock bluetooth
sudo systemctl restart bluetooth
echo ">>> Esperando firmware BT..."
sleep 1
sudo hciconfig hci0 up
# Forzamos Clase "No Audio" (Service Class: None, Major Device Class: Uncategorized)
sudo hciconfig hci0 class 0x000000

# ==========================================
# 4. RED (5GHz UNLOCK & SETUP)
# ==========================================
echo "=== [2/7] Configuración de Red ==="
if ! wait_for_interface "$WLAN_IF"; then
    echo ">>> FATAL: WiFi no apareció. Intentando recarga de driver..."
...
echo ">>> [UNLOCK] Iniciando escaneo para desbloquear 5GHz..."
...
# Escaneamos 3 veces CON TIMEOUT.
for i in {1..3}; do
    echo ">>> [UNLOCK] Scan attempt $i..."
    sudo timeout 3s iw dev $WLAN_IF scan > /dev/null 2>&1 || true
    sleep 0.5
done
echo ">>> [UNLOCK] Escaneo completado. Aplicando espera de seguridad..."

# Aumentamos el tiempo a 5s para que el Kernel termine de procesar "COUNTRY_UPDATE"
sleep 1
...
ieee80211ac=1
wmm_enabled=1
# Usamos HT20 para evitar conflictos de canal secundario
...
echo ">>> Iniciando Hostapd (5GHz)..."
sudo hostapd -B /tmp/hostapd.conf

# Check if Hostapd actually lived
sleep 2
if ! pgrep -x "hostapd" > /dev/null; then
    echo ">>> ERROR FATAL: Hostapd falló al iniciar. Revisa logs."
    exit 1
fi
...
echo "=== [6/7] Bluetooth Agent ==="
# NoInputNoOutput ayuda a que no pida PIN ni se crea teclado
...
echo "=== [7/7] LANZAMIENTO ==="
# Intentamos desactivar perfiles de voz una vez más por comando
